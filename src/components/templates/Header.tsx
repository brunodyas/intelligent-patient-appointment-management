...
const Header: React.FC<Header> = ({ children, title }) => {
  ...
  const currentActive = pathName && pathName[pathName.length - 1].replace(/\-+/g, ' ')

  return (
    <>
      <header className='HEADER HEADER-SANDBOX bg-neutral-20 py-16'>
        ...
        <section className='container relative mx-auto max-w-screen-xl space-y-6'>
          <nav className='flex items-center gap-2 px-6 py-1 capitalize leading-none text-neutral-60'>
            <Link href='/sandbox'>Components</Link>

            <ArrowRight2Icon className='h-3.5 w-3.5 stroke-2' />

            <Link className='font-semibold' href={path ? path: ''}>
              {currentActive}
            </Link>
          </nav>
