...
const Layout: React.FC<Layout> = ({ src, alt, desc, children }) => {
  ...
  return (
    <div className='Layout-Auth relative grid min-h-screen w-full grid-cols-12 bg-neutral-15'>
          ...
          {currentPath === "/auth/login" ? (
            ...
            <div className='mb-16 flex items-center gap-2'>
              <button type='button' onClick={() => router.back()}>
                <ArrowLeftIcon className='h-6 w-6 text-neutral-100' />
              </button>

              <Link href={"/auth/login"} className='flex items-center gap-2'>
                <NijaLogo className='h-7 w-7 2xl:h-8 2xl:w-8' />
                <h5 className='text-xl sm:text-body-xl font-semibold uppercase'>
                  Nija Kit
                </h5>
              </Link>
            </div>
