...
export default async function MyPage({ params }: { params: Params }) {
  ...
  const invoiceDetail = await fetchQuoteDetails(id)

  const invoiceInfo = {
    "Job Name": invoiceDetail?.job_name || "--",
    "Customer Name": invoiceDetail?.customer_name || "--",
    "Email": invoiceDetail?.customer_email || "--",
    "Phone": invoiceDetail?.customer_phone || "--",
    "Account Name": invoiceDetail?.account_name || "--",
    "Invoice Id": invoiceDetail?.invoice_id || "--",
    "Invoice Number": invoiceDetail?.number || "--",
    "Amount": invoiceDetail?.total?.length ? invoiceDetail.total : "--",
    "Created At": formatUnixTimestamp(invoiceDetail?.createdAt) ?? "--",
    "Due Date": formatUnixTimestamp(invoiceDetail?.due_date) ?? "--",
    "Status": invoiceDetail?.status || "--",
  };

  return (  
    <Fragment key={invoiceDetail?.job_name}>
      <Breadcrumb >
        <BreadcrumbList>
          <BreadcrumbItem>
            <Route
            ...
            <div className="grid gap-4 grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 py-5 text-sm">
              {Object.entries(invoiceInfo).map(([key, value]) => (
                <div key={key} className="flex flex-col">
                  ...
                  <p className={["Email", "Amount"].includes(key) ? "text-pink-500" : ""}>
                  {value}
                  </p>
                </div>
              ))}
      ...
      <QuoteItems pageView="invoice" jobId={id} />
    </Fragment>
  )
}
...
async function fetchQuoteDetails(id: string) {
  try {
    ...
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BE_URL}api/contacts/v1/view-invoice-details/${id}/`, {
      headers: {
